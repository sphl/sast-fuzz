...
#
# * llvm-config executable. This method is portable across LLVM build systems (i.e. works if LLVM was built with
#   autoconf/Makefile or with CMake).
#
# * find_package(LLVM CONFIG). This method only works if LLVM was built with CMake or with LLVM >= 3.5 when built with
#   the autoconf/Makefile build system This method relies on the `LLVMConfig.cmake` file generated to be generated by
#   LLVM's build system.
#
# ===----------------------------------------------------------------------------------------------------------------===
...
if (USE_CMAKE_FIND_PACKAGE_LLVM)
    ...
    # Use the llvm-config binary to get the information needed. Try to detect it in the user's environment. The user can
    # force a particular binary by passing `-DLLVM_CONFIG_BINARY=/path/to/llvm-config` to CMake.
    ...
    if ("${LLVM_PACKAGE_VERSION}" MATCHES "${_llvm_version_regex}")
        ...
        if ("${LLVM_PACKAGE_VERSION}" MATCHES "${_llvm_version_regex}")
            ...
            message(FATAL_ERROR "Failed to parse LLVM version from \"${LLVM_PACKAGE_VERSION}\"")
        endif ()
    endif ()
    ...
    foreach (flag ${_llvm_cxx_flags_list})
        if ("${flag}" STREQUAL "-DNDEBUG")
            # Note we don't rely on `llvm-config --build-mode` because that seems broken when LLVM is built with CMake.
    ...
    function (klee_get_llvm_libs OUTPUT_VAR)
        ...
        if (${LLVM_PACKAGE_VERSION} VERSION_LESS "3.5")
            # For LLVM 3.4 and older system libraries appeared in the output of `--ldflags`.
        ...
        # Create an imported target for each LLVM library if it doesn't already exist. We need to do this so we can tell
        # CMake that these libraries depend on the necessary libraries so that CMake can get the link order right.
        ...
        foreach (llvm_lib ${_llvm_libs_list})
            ...
            string(
                REGEX
                REPLACE "lib(libLLVM[-.a-zA-Z0-9]+\\.so)\\.so$"
                        "\\1"
                        llvm_lib
                        "${llvm_lib}"
            )
            ...
            if (NOT TARGET "${target_name}")
                ...
                set_property(TARGET "${target_name}" PROPERTY IMPORTED_LOCATION "${llvm_lib}")
            endif ()
        endforeach ()

        # Now state the dependencies of the created imported targets which we assume to be for each imported target the
        # libraries which appear after the library in `{_llvm_libs_list}` and then finally the system libs. It is
        # **essential** that we do this otherwise CMake will get the link order of the imported targets wrong.
        ...
        if ("${length_targets_to_return}" GREATER 0)
            ...
            foreach (llvm_target_lib ${created_targets})
                # DEBUG: message(STATUS "Adding deps for target ${llvm_target_lib}") Find position in
                # `targets_to_return`
                list(
                    FIND
                    targets_to_return
                ...
                if ("${position}" LESS "${targets_to_return_last_index}")
                    ...
                    foreach (index RANGE ${position_plus_one} ${targets_to_return_last_index})
                        ...
                        set_property(
                            TARGET "${llvm_target_lib}"
                            APPEND
                            PROPERTY INTERFACE_LINK_LIBRARIES "${target_for_index}"
                        )
                    endforeach ()
                endif ()
                ...
                set_property(
                    TARGET "${target_name}"
                    APPEND
                    PROPERTY INTERFACE_LINK_LIBRARIES "${_system_libs_list}"
                )
            endforeach ()
        endif ()
...
# Filter out `-DNEBUG` from LLVM_DEFINITIONS.  The caller can use `LLVM_ENABLE_ASSERTIONS` to decide how to set their
# defines.
