...
# ----------------------------------------------------------------------------------------------------------------------
#
# This file tries to find compilers to build LLVM bitcode. It is implicitly dependent on `find_llvm.cmake` already being
...
if (NOT EXISTS "${LLVM_DIS_TOOL}")
    message(FATAL_ERROR "The llvm-dis tool cannot be found at \"${LLVM_DIS_TOOL}\"")
endif ()
...
function (test_bitcode_compiler COMPILER SRC_EXT)
    ...
    execute_process(
        COMMAND "${COMPILER}" "-c" "-emit-llvm" "-o" "${BC_FILE}" "${SRC_FILE}"
    ...
    # Check if the LLVM version we are using is compatible with this compiler by invoking the `llvm-dis` tool on the
    # generated bitcode.
    set(LL_FILE "${SRC_FILE}.ll")
    execute_process(COMMAND "${LLVM_DIS_TOOL}" "-o" "${LL_FILE}" "${BC_FILE}" RESULT_VARIABLE LLVM_DIS_INVOKE_EXIT_CODE)
    ...
    # Remove temporary files. It's okay to not remove these on failure as they will be useful for developer debugging.
