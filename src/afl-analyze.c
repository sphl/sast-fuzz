...
static void show_legend(void) {
    SAYF("    " cLGR bgGRA " 01 " cRST " - no-op block              " cBLK bgLGN " 01 " cRST
         " - suspected length field\n"
         "    " cBRI bgGRA " 01 " cRST " - superficial content      " cBLK bgYEL " 01 " cRST
         " - suspected cksum or magic int\n"
         "    " cBLK bgCYA " 01 " cRST " - critical stream          " cBLK bgLRD " 01 " cRST
    ...
    for (i = 0; i < len; i++) {
        ...
        if (rtype == RESP_FIXED) {
            ...
            case 4: {
                ...
                if (val && (in_data[i] >> 7 != in_data[i + 1] >> 7 || in_data[i] >> 7 != in_data[i + 2] >> 7 ||
...
static void analyze(char **argv) {
    ...
    for (i = 0; i < in_len; i++) {
        ...
        if (prev_xff != xor_ff && prev_x01 != xor_01 && prev_s10 != sub_10 && prev_a10 != add_10) {
            seq_byte ^= 0x80;
        }
    ...
    if (exec_hangs) {
        WARNF(cLRD "Encountered %u timeouts - results may be skewed." cRST, exec_hangs);
    }
