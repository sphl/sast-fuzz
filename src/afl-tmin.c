...
static u8 run_target(char **argv, u8 *mem, u32 len, u8 first_run) {
    ...
    if (!child_pid) {
        ...
        if (dup2(use_stdin ? prog_in_fd : dev_null_fd, 0) < 0 || dup2(dev_null_fd, 1) < 0 || dup2(dev_null_fd, 2) < 0) {
    ...
    if (WIFSIGNALED(status) || (WIFEXITED(status) && WEXITSTATUS(status) == MSAN_ERROR) ||
...
static void minimize(char **argv) {
    ...
    OKF("Block normalization complete, %u byte%s replaced.", alpha_del0, alpha_del0 == 1 ? "" : "s");
...
next_del_blksize:
    ...
    OKF("Symbol minimization finished, %u symbol%s (%u byte%s) replaced.", syms_removed, syms_removed == 1 ? "" : "s",
    ...
    OKF("Character minimization done, %u byte%s replaced.", alpha_del2, alpha_del2 == 1 ? "" : "s");
...
finalize_all:
    ...
    SAYF("\n" cGRA "     File size reduced by : " cRST "%0.02f%% (to %u byte%s)\n" cGRA
         "    Characters simplified : " cRST "%0.02f%%\n" cGRA "     Number of execs done : " cRST "%u\n" cGRA
         "          Fruitless execs : " cRST "path=%u crash=%u hang=%s%u\n\n",
         100 - ((double)in_len) * 100 / orig_len, in_len, in_len == 1 ? "" : "s",
         ((double)(alpha_d_total)) * 100 / (in_len ? in_len : 1), total_execs, missed_paths, missed_crashes,
...
static void find_binary(u8 *fname) {
    ...
    if (strchr(fname, '/') || !(env_path = getenv("PATH"))) {
        ...
        if (stat(target_path, &st) || !S_ISREG(st.st_mode) || !(st.st_mode & 0111) || st.st_size < 4) {
...
int main(int argc, char **argv) {
    ...
    ACTF("Performing dry run (mem limit = %llu MB, timeout = %u ms%s)...", mem_limit, exec_tmout,
    ...
    if (!crash_mode) {
        OKF("Program terminates normally, minimizing in " cCYA "instrumented" cRST " mode.");
