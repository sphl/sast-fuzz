...
typedef struct Synergy {
    std::set<const llvm::Value *> OverlapVal, ContradictVal;
    std::set<std::pair<const llvm::Value *, unsigned>> OverlapRng, ContradictRng;
} Synergy;
...
class Interval {
  public:
    ...
    bool IsBot() const { return range_gt(getLB(), getUB(), isSigned); }

    llvm::APInt getLB() const { return bounds.first; }

    llvm::APInt getUB() const { return bounds.second; }

    std::pair<llvm::APInt, llvm::APInt> getBounds() const { return bounds; }
    ...
    unsigned getWidth() const { return bounds.first.getBitWidth(); }

    void setLB(llvm::APInt v) { bounds.first = v; }

    void setUB(llvm::APInt v) { bounds.second = v; }

  private:
    const bool isSigned;
    std::pair<llvm::APInt, llvm::APInt> bounds;

    void assignRhs(const Interval &rhs) { bounds = rhs.bounds; }
...
class AbstractState {
  ...
  public:
    ...
    const std::set<const llvm::Value *> &getValKeys() const { return valKeys; }

    const std::set<std::pair<const llvm::Value *, unsigned>> &getMemKeys() const { return memKeys; }

    Interval getRangeForValue(const llvm::Value *v) const;
    Interval getRangeForMem(std::pair<const llvm::Value *, unsigned>) const;

    Interval accessRangeForValue(const llvm::Value *v) const { return valRng.at(v); }
    ...
    Synergy getIntersect(const AbstractState &rhs) const;

    std::unordered_map<const llvm::Value *, Interval>::iterator val_begin() { return valRng.begin(); }

    std::unordered_map<const llvm::Value *, Interval>::iterator val_end() { return valRng.end(); }

    decltype(memRng)::iterator mem_begin() { return memRng.begin(); }

    decltype(memRng)::iterator mem_end() { return memRng.end(); }
};
