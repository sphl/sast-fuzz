...
class PreconditionFixpo {
  public:
    PreconditionFixpo(const llvm::Instruction *target, const ICFG &icfg, bool interAnalysis);
    void init();
    void run();

    void setTarget(const llvm::Instruction *target) { targetNode = getNodePre(target); }
    ...
    class FixpointDisjunctiveResult {
      ...
      public:
        ...
        decltype(synergyResult) getSynResult() const { return synergyResult; }
        ...
        void setValResult(const llvm::Instruction *v, APIntBounds bounds) { valResult[v] = std::move(bounds); }

        void setSynResult(const llvm::Instruction *v, std::pair<std::set<unsigned>, std::set<unsigned>> BugIDs) {
            synergyResult[v] = std::move(BugIDs);
            ;
        }
        ...
        void addArgRange(const llvm::Value *arg, const APIntBounds &bds) { argResult.insert({arg, bds}); }

        void addUnreachableBB(const llvm::BasicBlock *bb) { unreachableBBs.insert(bb); }

        void addConditionedBB(const llvm::BasicBlock *bb) { conditionedBBs.insert(bb); }
    };
  ...
  private:
    ...
    std::unordered_map<std::shared_ptr<ICFGNode>, llvm::SmallVector<SymExpr, 8>, ICFGNodeHash, ICFGNodeEqual>
            stateInMap;
    std::unordered_map<std::shared_ptr<ICFGNode>, llvm::SmallVector<SymExpr, 8>, ICFGNodeHash, ICFGNodeEqual>
            stateOutMap;
    std::unordered_map<std::shared_ptr<ICFGNode>,
                       std::map<unsigned, llvm::SmallVector<SymExpr, 8>>,
                       ICFGNodeHash,
                       ICFGNodeEqual>
            Contradict;
    std::unordered_map<std::shared_ptr<ICFGNode>,
    ...
    std::map<unsigned,
             std::unordered_map<std::shared_ptr<ICFGNode>, llvm::SmallVector<SymExpr, 8>, ICFGNodeHash, ICFGNodeEqual>>
            StateSummary;
    std::map<unsigned, std::set<std::shared_ptr<ICFGNode>>> joinedOutSummary;
    std::map<unsigned, std::set<std::shared_ptr<ICFGNode>>> joinedInSummary;

    std::set<std::shared_ptr<ICFGNode>> joinedLocIn;
    std::set<std::shared_ptr<ICFGNode>> joinedLocOut;

    unsigned CurID;
    // Multi-cluster could be considered for future
    std::map<const llvm::Value *, std::pair<std::set<unsigned>, std::set<unsigned>>> ResCluster;
    ...
    void pushProcessList(const ProcessItem &item) { processList.insert(item); }

    bool isProcessListEmpty() const { return processList.empty(); }
    ...
    void addPostCond(const std::shared_ptr<ICFGNode> &loc, const SymExpr &cond) { stateOutMap[loc].push_back(cond); }

    void addPreCond(const std::shared_ptr<ICFGNode> &loc, const SymExpr &cond) { stateInMap[loc].push_back(cond); }

    bool updateState(const std::shared_ptr<ICFGNode> &loc,
                     const SymExpr &cond,
                     bool isPostCond,
                     const llvm::BasicBlock *dst);

    bool updateStateOut(const std::shared_ptr<ICFGNode> &loc, const SymExpr &cond, const llvm::BasicBlock *dst);

    bool updateStateIn(const std::shared_ptr<ICFGNode> &loc, const SymExpr &cond);

    SymExpr getMemSymForPtr(const llvm::Value *ptr) const { return ldPtrToMemSym.at(ptr); }

    const llvm::Value *getPtrForMemSym(const SymExpr &sym) const { return memSymToLdPtr.find(sym)->second; }
    ...
    void initSyms();
    bool isValueTracked(const llvm::Value *v) const { return valSym.count(v); }
    ...
    void transferLocalControl(const std::shared_ptr<ICFGNode> &src,
