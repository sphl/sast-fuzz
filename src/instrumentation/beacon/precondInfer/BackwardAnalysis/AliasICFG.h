...
class ICFGNode {
  public:
    enum ICFGNodeKind { CALL_NODE, RET_NODE, ENTRY_NODE, EXIT_NODE, NORMAL_NODE };
    ...
    ICFGNodeKind getKind() const { return nodeK; }

    unsigned getID() const { return nodeID; }

    void addNext(std::shared_ptr<ICFGNode> nextN) { nextNodes.insert(nextN); }

    std::set<std::shared_ptr<ICFGNode>>::const_iterator next_begin() const { return nextNodes.begin(); }

    std::set<std::shared_ptr<ICFGNode>>::const_iterator next_end() const { return nextNodes.end(); }

    std::set<std::shared_ptr<ICFGNode>> getNextNodes() const { return nextNodes; }
...
template <ICFGNode::ICFGNodeKind K> class GenericICFGNode : public ICFGNode {
  public:
    GenericICFGNode(const llvm::Instruction *inst) : ICFGNode(inst, K) {}
    static bool classof(const ICFGNode *N) { return N->getKind() == K; }
};
...
class AliasAnalysisInterface {
  ...
  private:
    ...
    llvm::PassBuilder PB;
    llvm::AAManager AA;
    //    llvm::AnalysisManager<llvm::DominatorTreeAnalysis> BB;
...
//  class DominatorAnalysisInterface {
//  public:
//
//  };
...
class CGAnalysis {
  public:
    ...
    bool hasCallsitesForFun(const llvm::Function *callee) const { return calleeToCallsites.count(callee); }
    ...
    bool hasCalleesForCallsite(const llvm::Instruction *cs) const { return callsiteToCallees.count(cs); }

    llvm::SmallPtrSet<const llvm::Instruction *, 8> getReturnPointsForFun(const llvm::Function *fun) const {
        return funToRetInsts.at(fun);
    }

    bool hasReturnPointsForFun(const llvm::Function *fun) const { return funToRetInsts.count(fun); }
...
class ICFG {
  public:
    ...
    static bool isExitInst(const llvm::Instruction *inst) { return inst->getOpcode() == llvm::Instruction::Ret; }

    static bool isEdgeLegal(const std::shared_ptr<ICFGNode> &src, const std::shared_ptr<ICFGNode> &dst);

    const llvm::Instruction *getEntryInst(const llvm::Function *f) const { return &*(f->getEntryBlock().begin()); }

    const CGAnalysis &getCG() const { return CG; }

    const PointerAnalysisInterface &getPTA() const { return pa; }
  ...
  private:
    ...
    void exhaust(const ICFGNode *curNode, const std::stack<const ICFGNode *> &callStack) const;
