...
namespace {
llvm::cl::opt<bool> RelationOpt("use-relation-pattern", llvm::cl::init(true));
llvm::cl::opt<bool> DisableRefine("disable-refine", llvm::cl::init(false));
}  // namespace
...
static inline bool APINT_SIGNED_LESS(const llvm::APInt &a, const llvm::APInt &b) { return a.slt(b); }

RangeAnalysis::RangeAnalysis(const ExprTy &cond) : condition(cond) { assert(condition->getWidth() == 1); }
...
void RangeAnalysis::patternMatch(const ExprTy &e) {
    auto isCmp = [](const ExprTy &exp) {
        auto K = exp->getKind();
        return K == Expr::Ult || K == Expr::Ule || K == Expr::Slt || K == Expr::Sle || K == Expr::Ne ||
               K == Expr::Ugt || K == Expr::Uge || K == Expr::Sgt || K == Expr::Sge;
    };
...
void RangeAnalysis::patternMatch(const ExprTy &e, bool isTrue) {
    ...
    auto isVarOrConst = [](const ExprTy &exp) {
        auto K = exp->getKind();
        return K == Expr::Read || (K == Expr::Extract && exp->getWidth() == 1) || K == Expr::Concat ||
               K == Expr::Constant;
    };
    ...
    if (isVarOrConst(leftOp) && isVarOrConst(rightOp)) {  // x cmp y
        auto cond1 =
                makeCmpExpr(cmpKind, SubExpr::create(leftOp, rightOp), ConstantExpr::create(0, leftOp->getWidth()));
        auto cond2 = makeCmpExpr(getRevPred(cmpKind), SubExpr::create(rightOp, leftOp),
                                 ConstantExpr::create(0, leftOp->getWidth()));
...
void RangeAnalysis::propagateUptoParent(const ExprTy &e,
                                        const klee::ref<klee::SelectExpr> &parent,
                                        const Interval &itv) {
...
void RangeAnalysis::propagateUptoParent(const ExprTy &e,
                                        const klee::ref<klee::ExtractExpr> &parent,
                                        const Interval &itv) {
...
void RangeAnalysis::propagateUptoParentBinary(const ExprTy &e,
                                              const klee::ref<klee::BinaryExpr> &parent,
                                              const Interval &itv) {
...
bool RangeAnalysis::isExprVar(const ExprTy &e) const {
    return e->getKind() == Expr::Concat || e->getKind() == Expr::Read || e->getKind() == Expr::Extract;
}

bool RangeAnalysis::isExprConst(const ExprTy &e) const { return e->getKind() == Expr::Constant; }
...
Expr::Kind RangeAnalysis::getRevPred(Expr::Kind pred) const {
    static const std::map<Expr::Kind, Expr::Kind> lookup{
            std::make_pair(Expr::Eq, Expr::Eq),   std::make_pair(Expr::Ne, Expr::Ne),

            std::make_pair(Expr::Ult, Expr::Ugt), std::make_pair(Expr::Ugt, Expr::Ult),

            std::make_pair(Expr::Ule, Expr::Uge), std::make_pair(Expr::Uge, Expr::Ule),

            std::make_pair(Expr::Slt, Expr::Sgt), std::make_pair(Expr::Sgt, Expr::Slt),

            std::make_pair(Expr::Sle, Expr::Sge), std::make_pair(Expr::Sge, Expr::Sle),
    };
...
klee::Expr::Kind RangeAnalysis::getInversePred(klee::Expr::Kind pred) const {
    static const std::map<Expr::Kind, Expr::Kind> lookup{
            std::make_pair(Expr::Eq, Expr::Ne),   std::make_pair(Expr::Ne, Expr::Eq),

            std::make_pair(Expr::Ult, Expr::Uge), std::make_pair(Expr::Uge, Expr::Ult),

            std::make_pair(Expr::Ule, Expr::Ugt), std::make_pair(Expr::Ugt, Expr::Ule),

            std::make_pair(Expr::Slt, Expr::Sge), std::make_pair(Expr::Sge, Expr::Slt),

            std::make_pair(Expr::Sle, Expr::Sgt), std::make_pair(Expr::Sgt, Expr::Sle),
    };
...
void RangeAnalysis::propagateDowntoKid(const klee::ref<klee::ConcatExpr> &e, const Interval &itv) { return; }
...
Interval RangeAnalysis::itvDiv(const Interval &I1, const Interval &I2, bool isSignedOp) const {
    ...
    Interval resItv(I1);
    resItv.setLB(*std::min_element(extremes.begin(), extremes.end(), APINT_SIGNED_LESS));
    resItv.setUB(*std::max_element(extremes.begin(), extremes.end(), APINT_SIGNED_LESS));
...
APInt minOr(const APInt &a, const APInt &b, const APInt &c, const APInt &d) {
    APInt res(a.getBitWidth(), minOr_int64t(a.getSExtValue(), b.getSExtValue(), c.getSExtValue(), d.getSExtValue()));
    return res;
}

APInt maxOr(const APInt &a, const APInt &b, const APInt &c, const APInt &d) {
    APInt res(a.getBitWidth(), maxOr_int64t(a.getSExtValue(), b.getSExtValue(), c.getSExtValue(), d.getSExtValue()));
    return res;
}
...
APInt minXor(const APInt &a, const APInt &b, const APInt &c, const APInt &d) {
    return (minAnd(a, b, ~d, ~c) | minAnd(~b, ~a, c, d));
}

APInt maxXor(const APInt &a, const APInt &b, const APInt &c, const APInt &d) {
    return (maxOr(APInt::getNullValue(a.getBitWidth()), maxAnd(a, b, ~d, ~c), APInt::getNullValue(a.getBitWidth()),
                  maxAnd(~b, ~a, c, d)));
}
...
Interval RangeAnalysis::itvOr(const Interval &I1, const Interval &I2) const {
    ...
    case 5:  // - + - +
        lb = smin(a, c);
        ub = maxOr(APInt::getNullValue(width), b, APInt::getNullValue(width), d);
        break;
    case 7:  // - + + +
...
Interval RangeAnalysis::itvCmp(const Interval &I1, const Interval &I2, klee::Expr::Kind cmpPred) const {
    ...
    if (std::set<Expr::Kind>{Expr::Eq, Expr::Ult, Expr::Ule, Expr::Slt, Expr::Sle}.count(cmpPred)) {
        if (cmpPred == Expr::Eq) {
            ...
            auto resItv = std::accumulate(std::next(candidates.begin()), candidates.end(), candidates[0],
...
Interval RangeAnalysis::intervalConst(const llvm::APInt &v) const { return Interval({v, v}, true); }

Interval RangeAnalysis::intervalConst(int64_t v, unsigned w) const { return intervalConst(llvm::APInt(w, v)); }

Interval RangeAnalysis::intervalConst(bool c) const { return intervalConst(c, 1); }

Interval RangeAnalysis::intervalTop(unsigned w) const { return Interval::createTop(w, true); }

Interval RangeAnalysis::intervalBottom(unsigned w) const { return Interval::createBottom(w, true); }
...
bool RangeAnalysis::isIntervalTop(const Interval &itv) const { return itv.IsTop(); }

bool RangeAnalysis::isIntervalBot(const Interval &itv) const { return itv.IsBot(); }
...
Interval RangeAnalysis::refineIntervalLess(const Interval &lhs, const Interval &rhs, Expr::Kind cmpPred) const {
    ...
    if (cmpPred == Expr::Slt || cmpPred == Expr::Sle) {  // apply signed comparison
        ...
        resItv = std::accumulate(std::next(candidates.begin()), candidates.end(), candidates[0],
                                 [](const Interval &I1, const Interval &I2) {
                                     Interval res(I1);
                                     res.join(I2);
                                     return res;
...
Interval RangeAnalysis::getRange(const ExprTy &e) const { return result.find(e)->second; }
...
bool RangeCompute::isCondUnSat() { return isUnsat; }
...
RangeCompute::getRanges(const std::vector<klee::ref<klee::Expr>> &syms) {
...
RangeCompute::~RangeCompute() { delete impl; }
