...
cl::opt<bool> EnableFunctionTypeMatching("fp-type-match", cl::init(false));

cl::opt<bool> UseCheapAliasAnalysis("use-cheap-alias", cl::init(false));

unsigned ICFGNode::ICFGNodeID = 0;
ICFGNode::ICFGNode(const Instruction *inst, ICFGNodeKind k) : inst(inst), nodeK(k), nodeID(ICFGNodeID++) {}

void CGAnalysis::build(llvm::Module *m) {
    auto isCalleeSuitable = [](const Function *fun) { return !fun->empty(); };
    ...
    for (Module::iterator i = m->begin(), ie = m->end(); i != ie; ++i) {
        ...
        for (inst_iterator iter = inst_begin(*f), eiter = inst_end(*f); iter != eiter; ++iter) {
            ...
            /// %30 = call i32 bitcast (i64 (%struct._IO_FILE*)* @readUInt32 to i32
            /// (%struct._IO_FILE*)*)(%struct._IO_FILE* %29)
            if (ICFG::isCallInst(inst)) {
                SVF::CallSite cs(inst);
...
CGAnalysis::CGAnalysis(Module *m, const PointerAnalysisInterface *pa) : pa(pa) { build(m); }
...
AliasAnalysisInterface::AliasAnalysisInterface() {
    ...
    //  PB.registerFunctionAnalyses(BB);
    //  PB.registerFunctionAnalyses(llvm::DominatorTreeWrapperPass);
}
...
PointerAnalysisInterface::PointerAnalysisInterface(llvm::Module *M) {
    if (!UseCheapAliasAnalysis) {
        ...
        SVFMod = SVF::LLVMModuleSet::getLLVMModuleSet()->buildSVFModule(*M);

        SVF::PAGBuilder builder;
        SVF::PAG *pag = builder.build(SVFMod);
...
std::set<const llvm::Function *> PointerAnalysisInterface::getIndCallees(llvm::Instruction *inst) const {
    ...
    // TODO: Check if the code below is correct!
    SVF::CallBlockNode *cs = pta->getPAG()->getICFG()->getCallBlockNode(inst);

    auto ptCG = pta->getPTACallGraph();

    if (ptCG->hasIndCSCallees(cs)) {
        std::set<const llvm::Function *> funcSet;
        for (auto callee : ptCG->getIndCSCallees(cs)) {
            funcSet.insert(callee->getLLVMFun());
        }
        return funcSet;
    } else {
        return getCalleesByTypeMatching(inst);
...
static bool isFunctionTypeMatched(const llvm::FunctionType *srcTy, const llvm::FunctionType *dstTy) {
    ...
    for (unsigned i = 0; i < numParams; ++i) {
        ...
        //
        //      // check starts with relation: %struct.jpeg_compress_struct.170* and %struct.jpeg_compress_struct are
        //      the same struct if (srcParamName.rfind(dstParamName, 0) == 0 ||
        ...
        //        if (srcParamElemTy->getNumContainedTypes() == 1 && srcParamElemTy->getContainedType(0) ==
        //        dstParamElemTy) {
        //          continue;
        //        }
        //
        //        if (dstParamElemTy->getNumContainedTypes() == 1 && dstParamElemTy->getContainedType(0) ==
        //        srcParamElemTy) {
        //          continue;
        //        }
        //
...
ICFG::ICFG(Module *m, bool isReversed) : m(m), isReversed(isReversed), pa(m), CG(m, &pa) { build(); }
...
bool ICFG::isLoopExitBB(const llvm::BasicBlock *bb) const { return loopExitBBs.count(bb); }

bool ICFG::isLoopBB(const llvm::BasicBlock *bb) const { return loopBBs.count(bb); }
    ...
    while (!localDfsStack.empty()) {
        ...
        if (nodeK == ICFGNode::RET_NODE) {
            ...
            for (const auto &nextN : node->getNextNodes()) {
                ...
                //        } else {
                //          if (callStack.top()->getInst() == nextN->getInst()) {
                //            assert(callStack.top()->getKind() == ICFGNode::RET_NODE && "Recursive calls should be
                //            handled"); newCallStack = poppedCallStack(callStack);
...
ICFG::dfsWithCFL(const ICFGNode *curNode, const std::stack<const ICFGNode *> &callStack) const {
    ...
    auto addToNext =
            [](std::vector<std::pair<const ICFGNode *, std::pair<const ICFGNode *, std::stack<const ICFGNode *>>>>
                       &next,
               const ICFGNode *node, const std::stack<const ICFGNode *> &calls,
               const ICFGNode *prevNode) { next.emplace_back(std::make_pair(prevNode, std::make_pair(node, calls))); };
...
std::shared_ptr<ICFGNode>
ICFG::getOrCreateNormalNode(const Instruction *inst) {  // may be call inst with no resolvable callees
