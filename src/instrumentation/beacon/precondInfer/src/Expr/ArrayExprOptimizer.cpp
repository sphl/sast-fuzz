...
namespace klee {
llvm::cl::opt<ArrayOptimizationType>
        OptimizeArray("optimize-array",
                      llvm::cl::values(clEnumValN(ALL, "all", "Combining index and value transformations"),
                                       clEnumValN(INDEX, "index", "Index-based transformation"),
                                       clEnumValN(VALUE,
                                                  "value",
                                                  "Value-based transformation at branch (both "
                                                  "concrete and concrete/symbolic)") KLEE_LLVM_CL_VAL_END),
                      llvm::cl::init(NONE),
                      llvm::cl::desc("Optimize accesses to either concrete or concrete/symbolic "
                                     "arrays. (default=off)"));

llvm::cl::opt<double> ArrayValueRatio("array-value-ratio",
...
llvm::cl::opt<double> ArrayValueSymbRatio("array-value-symb-ratio",
...
ref<Expr> extendRead(const UpdateList &ul, const ref<Expr> index, Expr::Width w) {
    ...
    case Expr::Int16:
        return ConcatExpr::create(ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(1, Expr::Int32), index)),
                                  ReadExpr::alloc(ul, index));
    case Expr::Int32:
        return ConcatExpr::create4(ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(3, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(2, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(1, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, index));
    case Expr::Int64:
        return ConcatExpr::create8(ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(7, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(6, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(5, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(4, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(3, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(2, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, AddExpr::create(ConstantExpr::create(1, Expr::Int32), index)),
                                   ReadExpr::alloc(ul, index));
    }
...
ref<Expr> ExprOptimizer::optimizeExpr(const ref<Expr> &e, bool valueOnly) {
    ...
    if (OptimizeArray == VALUE || (OptimizeArray == ALL && (!result.get() || valueOnly))) {
        ...
        ref<Expr> selectOpt = getSelectOptExpr(e, reads, readInfo, are.containsSymbolic());
        if (selectOpt.get()) {
    ...
    for (auto &element : arrays) {
        ...
        for (size_t aIdx = 0; aIdx < arr->constantValues.size(); aIdx += width) {
            ...
            for (auto &index_it : element.second) {
                ...
                bool assignmentSuccess = AssignmentGenerator::generatePartialAssignment(idx, val, a);
                ...
                if (assignmentSuccess && evaluation->isTrue()) {
                    ...
                    idx_valIdx[idx].emplace_back(ConstantExpr::alloc(aIdx, arr->getDomain()));
                }
            }
            delete a;
        }
...
ref<Expr> ExprOptimizer::getSelectOptExpr(const ref<Expr> &e,
    ...
    if (!isSymbolic) {
        ...
        for (auto &read : reads) {
            ...
            assert(read->updates.root->isConstantArray() && "Expected concrete array, found symbolic array");
            ...
            for (unsigned i = 0; i < elementsInArray; i++) {
                ...
                for (unsigned j = 0; j < bytesPerElement; j++) {
                    val |= (*(arrayConstValues[(i * bytesPerElement) + j].get()->getAPValue().getRawData()) << (j * 8));
                }
            ...
            ref<Expr> opt = buildConstantSelectExpr(index, arrayValues, width, elementsInArray);
            if (opt.get()) {
    ...
    //       array is symbolic && updatelist contains at least one concrete value
        ...
        for (auto &read : reads) {
            ...
            for (unsigned i = 0; i < elementsInArray; i++) {
                ...
                for (unsigned j = 0; j < bytesPerElement; j++) {
                    if (ba.get((i * bytesPerElement) + j)) {
                        ...
                        val |= (*(arrayConstValues[(i * bytesPerElement) + j].get()->getAPValue().getRawData())
                                << (j * 8));
                    }
                }
            ...
            if (((double)symByteNum / (double)elementsInArray) <= ArrayValueSymbRatio) {
                ...
                ref<Expr> opt = buildMixedSelectExpr(read, arrayValues, width, elementsInArray);
                if (opt.get()) {
                    cacheReadExprOptimized[read->hash()] = opt;
                    optimized.insert(std::make_pair(info.first, opt));
                }
...
ref<Expr> ExprOptimizer::buildConstantSelectExpr(const ref<Expr> &index,
    ...
    for (size_t i = 0; i < ranges.size(); i++) {
        if (exprMap.find(values[i]) != exprMap.end()) {
            ...
            if (exprMap.find(values[i]) == exprMap.end()) {
                exprMap.insert(std::make_pair(values[i], std::vector<std::pair<uint64_t, uint64_t>>()));
            }
            exprMap.find(values[i])->second.emplace_back(ranges[i].first, ranges[i].second);
        }
    }
                ...
                if (range.second[0].first == (range.second[0].second - 1)) {
                    temp = SelectExpr::create(
                            EqExpr::create(actualIndex,
                                           builder->Constant(llvm::APInt(idxWidth, range.second[0].first, false))),
                            builder->Constant(llvm::APInt(valWidth, range.first, false)), result);

                } else {
                    temp = SelectExpr::create(
                            AndExpr::create(
                                    SgeExpr::create(actualIndex, builder->Constant(llvm::APInt(
                                                                         idxWidth, range.second[0].first, false))),
                                    SltExpr::create(actualIndex, builder->Constant(llvm::APInt(
                ...
                if (range.second[0].first == (range.second[0].second - 1)) {
                    ...
                    currOr = AndExpr::create(
                            ...
                            SltExpr::create(actualIndex,
                                            builder->Constant(llvm::APInt(idxWidth, range.second[0].second, false))));
                }
                for (size_t i = 1; i < range.second.size(); i++) {
                    ...
                    if (range.second[i].first == (range.second[i].second - 1)) {
                        tempOr = OrExpr::create(
                                EqExpr::create(actualIndex,
                                               builder->Constant(llvm::APInt(idxWidth, range.second[i].first, false))),
                                currOr);
                        ...
                        tempOr = OrExpr::create(
                                AndExpr::create(
                                        ...
                                        SltExpr::create(actualIndex,
                                                        builder->Constant(
                                                                llvm::APInt(idxWidth, range.second[i].second, false)))),
                                currOr);
                    }
                    currOr = tempOr;
                }
                temp = SelectExpr::create(currOr, builder->Constant(llvm::APInt(valWidth, range.first, false)), result);
            }
        }
        result = temp;
        ct++;
...
ref<Expr> ExprOptimizer::buildMixedSelectExpr(const ReadExpr *re,
    ...
    if (((double)unique_array_values.size() / (double)(arraySize)) <= ArrayValueRatio) {
        ...
        if (holes.empty()) {
            ...
            ref<Expr> firstIndex = MulExpr::create(ConstantExpr::create(holes[0], re->index->getWidth()),
                                                   ConstantExpr::create(width / 8, re->index->getWidth()));
            result = extendRead(re->updates, firstIndex, width);
            for (size_t i = 1; i < holes.size(); i++) {
                ref<Expr> temp_idx = MulExpr::create(ConstantExpr::create(holes[i], re->index->getWidth()),
                                                     ConstantExpr::create(width / 8, re->index->getWidth()));
                ref<Expr> cond = EqExpr::create(re->index, ConstantExpr::create(holes[i], re->index->getWidth()));
                ref<Expr> temp = SelectExpr::create(cond, extendRead(re->updates, temp_idx, width), result);
                result = temp;
            }
        }
        ...
        for (size_t i = range_start; i < ranges.size(); i++) {
            ...
            if (ranges[i].second - 1 == ranges[i].first) {
                ref<Expr> cond = EqExpr::create(new_index, ConstantExpr::create(ranges[i].first, new_index_width));
                ...
                ref<Expr> cond = AndExpr::create(
                        SgeExpr::create(new_index, ConstantExpr::create(ranges[i].first, new_index_width)),
                        SltExpr::create(new_index, ConstantExpr::create(ranges[i].second, new_index_width)));
                ref<Expr> t = ConstantExpr::create(values[i], width);
                ref<Expr> f = result;
                temp = SelectExpr::create(cond, t, f);
            }
