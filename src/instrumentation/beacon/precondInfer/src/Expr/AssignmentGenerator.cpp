...
bool AssignmentGenerator::generatePartialAssignment(const ref<Expr> &e, ref<Expr> &val, Assignment *&a) {
    return helperGenerateAssignment(e, val, a, e->getWidth(), false);
}

bool AssignmentGenerator::helperGenerateAssignment(
        const ref<Expr> &e, ref<Expr> &val, Assignment *&a, Expr::Width width, bool sign) {
    Expr &ep = *e.get();
    switch (ep.getKind()) {
    // ARITHMETIC
    case Expr::Add: {
    ...
    case Expr::Concat: {
        ...
        if (base) {
            return helperGenerateAssignment(ref<Expr>(base), val, a, ep.getWidth(), sign);
        } else {
    ...
    case Expr::Read: {
        ...
        if (isa<ConstantExpr>(re.index)) {
            if (re.updates.root->isSymbolicArray()) {
                ...
                std::vector<unsigned char> c_value = getIndexedValue(getByteValue(val), index, re.updates.root->size);
                if (c_value.size() == 0) {
                    return false;
                }
...
bool AssignmentGenerator::isReadExprAtOffset(ref<Expr> e, const ReadExpr *base, ref<Expr> offset) {
...
ref<Expr> AssignmentGenerator::createSubExpr(const ref<Expr> &l, ref<Expr> &r) { return SubExpr::create(r, l); }

ref<Expr> AssignmentGenerator::createAddExpr(const ref<Expr> &l, ref<Expr> &r) { return AddExpr::create(r, l); }

ref<Expr> AssignmentGenerator::createMulExpr(const ref<Expr> &l, ref<Expr> &r) { return MulExpr::create(r, l); }

ref<Expr> AssignmentGenerator::createDivRem(const ref<Expr> &l, ref<Expr> &r, bool sign) {
    if (sign)
...
ref<Expr> AssignmentGenerator::createDivExpr(const ref<Expr> &l, ref<Expr> &r, bool sign) {
    if (sign)
...
ref<Expr> AssignmentGenerator::createShlExpr(const ref<Expr> &l, ref<Expr> &r) { return ShlExpr::create(r, l); }

ref<Expr> AssignmentGenerator::createLShrExpr(const ref<Expr> &l, ref<Expr> &r) { return LShrExpr::create(r, l); }

ref<Expr> AssignmentGenerator::createAndExpr(const ref<Expr> &l, ref<Expr> &r) { return AndExpr::create(r, l); }

ref<Expr> AssignmentGenerator::createExtractExpr(const ref<Expr> &l, ref<Expr> &r) {
    return ExtractExpr::create(r, 0, l.get()->getWidth());
}

ref<Expr> AssignmentGenerator::createExtendExpr(const ref<Expr> &l, ref<Expr> &r) {
