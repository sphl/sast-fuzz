...
class ExprReplaceVisitor2 : public ExprVisitor {
  ...
  public:
    ...
    Action visitExprPost(const Expr &e) {
        std::map<ref<Expr>, ref<Expr> >::const_iterator it = replacements.find(ref<Expr>(const_cast<Expr *>(&e)));
...
bool ConstraintManager::rewriteConstraints(ExprVisitor &visitor) {
    ...
    for (ConstraintManager::constraints_ty::iterator it = old.begin(), ie = old.end(); it != ie; ++it) {
        ref<Expr> &ce = *it;
...
ref<Expr> ConstraintManager::simplifyExpr(ref<Expr> e) const {
    ...
    for (ConstraintManager::constraints_ty::const_iterator it = constraints.begin(), ie = constraints.end(); it != ie;
         ++it) {
