...
compressed_fd_ostream::compressed_fd_ostream(const std::string &Filename, std::string &ErrorInfo)
    ...
    std::error_code EC = llvm::sys::fs::openFileForWrite(Filename, FD, llvm::sys::fs::F_None);
#else
    llvm::error_code EC = llvm::sys::fs::openFileForWrite(Filename, FD, llvm::sys::fs::F_Binary);
#endif
    if (EC) {
...
void compressed_fd_ostream::write_impl(const char *Ptr, size_t Size) {
    strm.next_in = const_cast<unsigned char *>(reinterpret_cast<const unsigned char *>(Ptr));
    strm.avail_in = Size;
