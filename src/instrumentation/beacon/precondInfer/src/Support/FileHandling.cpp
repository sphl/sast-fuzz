...
std::unique_ptr<llvm::raw_fd_ostream> klee_open_output_file(const std::string &path, std::string &error) {
    ...
    f = std::unique_ptr<llvm::raw_fd_ostream>(
            new llvm::raw_fd_ostream(path.c_str(), ec, llvm::sys::fs::F_None));  // FIXME C++14
    if (ec)
        error = ec.message();
#elif LLVM_VERSION_CODE >= LLVM_VERSION(3, 5)
    f = std::unique_ptr<llvm::raw_fd_ostream>(
            new llvm::raw_fd_ostream(path.c_str(), error, llvm::sys::fs::F_None));  // FIXME C++14
#else
    f = std::unique_ptr<llvm::raw_fd_ostream>(
            new llvm::raw_fd_ostream(path.c_str(), error, llvm::sys::fs::F_Binary));  // FIXME C++14
#endif
    if (!error.empty()) {
        f.reset(nullptr);
    }
...
#ifdef HAVE_ZLIB_H
std::unique_ptr<llvm::raw_ostream> klee_open_compressed_output_file(const std::string &path, std::string &error) {
    error = "";
