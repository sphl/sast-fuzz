...
TreeStreamWriter::TreeStreamWriter(const std::string &_path)
    : lastID(0), bufferCount(0), path(_path), output(new std::ofstream(path.c_str(), std::ios::out | std::ios::binary)),
      ids(1) {
    if (!output->good()) {
        delete output;
        output = 0;
...
void TreeStreamWriter::readStream(TreeStreamID streamID, std::vector<unsigned char> &out) {
...
TreeOStream::TreeOStream(TreeStreamWriter &_writer, unsigned _id) : writer(&_writer), id(_id) {}
