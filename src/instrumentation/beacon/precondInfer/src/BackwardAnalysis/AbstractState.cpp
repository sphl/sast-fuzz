...
Interval::Interval(std::pair<llvm::APInt, llvm::APInt> bounds, bool isSigned)
    ...
    /// FIXME: can not call normalize here, will trigger infinite recursion. Should be fine, check all external Interval
    /// construction calls :)
}

Interval::Interval(const Interval &rhs) : isSigned(rhs.isSigned), bounds(rhs.bounds) {}
...
bool AbstractState::IsBot() const { return valRng.empty() && memRng.empty(); }
...
Synergy AbstractState::getIntersect(const AbstractState &rhs) const {
    Synergy res;

    printf("enter intersect???\n");
    if (rhs.IsBot()) {
        res.OverlapVal = getValKeys();
        res.OverlapRng = getMemKeys();

        return res;
    }

    if (IsBot()) {
        res.OverlapVal = rhs.getValKeys();
        res.OverlapRng = rhs.getMemKeys();
        return res;
    }

    printf("no123 intersect???\n");
    for (auto iter = valRng.begin(); iter != valRng.end(); ++iter) {
        const Value *val = iter->first;

        // val->dump();

        auto lr = getRangeForValue(val);
        auto rr = rhs.getRangeForValue(val);
        if (lr.isLessOrEqual(rr) || rr.isLessOrEqual(lr) || Interval::hasIntersection(lr, rr)) {
            res.OverlapVal.insert(val);
        } else if (!Interval::hasIntersection(lr, rr)) {
            res.ContradictVal.insert(val);
        } else {
            printf("no intersect???\n");
        }
    }
    ...
    for (auto &objWidthRange : memRng) {
        auto key = objWidthRange.first;
        auto lr = getRangeForMem(key);
        auto rr = rhs.getRangeForMem(key);
        if (lr.isLessOrEqual(rr) || rr.isLessOrEqual(lr) || Interval::hasIntersection(lr, rr)) {
            res.OverlapRng.insert(key);
        } else if (!Interval::hasIntersection(lr, rr)) {
            res.ContradictRng.insert(key);
        }
    }
...
DisjunctiveIntervalSummary::DisjunctiveInterval
DisjunctiveIntervalSummary::addToDisjointIntervals(const DisjunctiveIntervalSummary::DisjunctiveInterval &s,
                                                   Interval itv) {
    assert(!s.empty());
    ...
    DisjunctiveInterval res;
    Interval mergedR = std::accumulate(intersectItvs.begin(), intersectItvs.end(), itv, [](Interval r1, Interval r2) {
        r1.join(r2);
        return r1;
    });
    ...
    std::sort(res.begin(), res.end(),
              [](const Interval &l, const Interval &r) { return l.getLB().getSExtValue() < r.getLB().getSExtValue(); });
    ...
    DisjunctiveInterval foldedResult =
            std::accumulate(std::next(foldContainer.begin()), foldContainer.end(), foldContainer[0],
...
DisjunctiveIntervalSummary::APIntBounds DisjunctiveIntervalSummary::getRangesForVal(const llvm::Value *V,
                                                                                    bool &isTop) const {
    APIntBounds result;
    if (!summary.count(V)) {
        isTop = true;
    ...
    std::transform(rngs.begin(), rngs.end(), std::back_inserter(result),
                   [](const Interval &r) { return r.getBounds(); });
...
std::unordered_map<const llvm::Value *, DisjunctiveIntervalSummary::APIntBounds>
DisjunctiveIntervalSummary::getRangesForArgs() const {
