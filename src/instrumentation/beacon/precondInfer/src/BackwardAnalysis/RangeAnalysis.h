...
class RangeAnalysis {
  public:
    ...
    void analyze();
    bool hasContradiction() const { return goesToBot; }
  ...
  private:
    ...
    klee::ExprHashMap<unsigned> wlCount;
    ...
    bool wlEmpty() const { return worklist.empty(); }
    ...
    /// parent can never be concat or extract expr. We view them as a whole, thus their sub expressions can never
    /// update.
