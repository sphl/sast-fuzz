...
std::string disjointRangeSetToStr(const std::vector<std::pair<llvm::APInt, llvm::APInt>> &s) {
    ...
    auto res = std::accumulate(std::next(rangeStrs.begin()), rangeStrs.end(), rangeStrs[0],
...
WPInferenceInterface::WPInferenceInterface(Module *M, bool isInterProc) : M(M), isInterAnalysis(isInterProc) {
...
void WPInferenceInterface::setTargets(std::set<const llvm::Instruction *> targets) {
...
WPInferenceInterface::~WPInferenceInterface() { delete icfg; }
...
void WPInferenceInterface::dumpReachBBs(const std::set<const llvm::BasicBlock *> &targetBBs) {
    ...
    std::string targetfile = TargetFile.getValue();
    std::replace(targetfile.begin(), targetfile.end(), '/', '_');
    targetfile = "bbreaches_" + targetfile;

    std::error_code EC;
    llvm::raw_fd_ostream oss(targetfile, EC, sys::fs::F_None);
...
void WPInferenceInterface::runForTarget(const Instruction *target, bool useSolver) {
    ...
    auto fromFixpoResult = [](const PreconditionFixpo::FixpointDisjunctiveResult &fixpoRes) {
        return FunctionResult(fixpoRes.getArgResult(), fixpoRes.getValResult(), fixpoRes.getSynResult(),
                              fixpoRes.getConditionedBBs(), fixpoRes.getUnreachableBBs());
    };

    static PreconditionFixpo fixpo(target, *icfg, isInterAnalysis);
    fixpo.setTarget(target);
    fixpo.init();
    fixpo.run();

    std::unordered_map<const llvm::Function *, PreconditionFixpo::FixpointDisjunctiveResult> analysisRes =
            fixpo.getAnalysisResult();
    ...
    for (const auto &funRes : analysisRes) {
        ...
        if (analysisResultByFun.find(fun) != analysisResultByFun.end()) {
            analysisResultByFun[fun] = fromFixpoResult(fixpoResult);
        } else {
        ...
        syn += fixpoResult.getSynResult().size();
    }
...
void WPInferenceInterface::dumpResult() {
    ...
    for (auto iter = analysisResultByFun.begin(); iter != analysisResultByFun.end(); ++iter) {
        ...
        rapidjson::Value funNameV;
        funNameV.SetString(funName.c_str(), funName.size(), allocator);
        rapidjson::Value funLocV;
        funLocV.SetUint(getAsmLineAtInst(firstInstOfFunc));
        funRecord.AddMember("fun", funNameV, allocator);
        funRecord.AddMember("funLoc", funLocV, allocator);

        rapidjson::Value bugClusterArr(rapidjson::kArrayType);
        for (const auto &inst : funRes.synergyResult) {
            rapidjson::Value bugCluster(rapidjson::kObjectType);
            rapidjson::Value instNameV;
            instNameV.SetString(llvmValToStr(inst.first).c_str(), llvmValToStr(inst.first).size(), allocator);
            rapidjson::Value locV;
            locV.SetUint(getAsmLineAtInst(inst.first));

            rapidjson::Value group1(rapidjson::kArrayType);
            for (auto bugID : inst.second.first) {
                rapidjson::Value bug;
                bug.SetUint(bugID);
                group1.PushBack(bug, allocator);
            }

            rapidjson::Value group2(rapidjson::kArrayType);
            for (auto bugID : inst.second.second) {
                rapidjson::Value bug;
                bug.SetUint(bugID);
                group2.PushBack(bug, allocator);
            }

            bugCluster.AddMember("value", instNameV, allocator);
            bugCluster.AddMember("line", locV, allocator);
            bugCluster.AddMember("true", group1, allocator);
            bugCluster.AddMember("false", group2, allocator);

            bugClusterArr.PushBack(bugCluster, allocator);
        }

        funRecord.AddMember("synRes", bugClusterArr, allocator);
        ...
        for (const auto &instRng : funRes.valResult) {
            ...
            rapidjson::Value instNameV;
            instNameV.SetString(instName.c_str(), instName.size(), allocator);
            rapidjson::Value rangeV;
            rangeV.SetString(rangeRes.c_str(), rangeRes.size(), allocator);
            rapidjson::Value lineV;
            lineV.SetUint(getAsmLineAtInst(instRng.first));
        ...
        for (const auto &argRng : funRes.argResult) {
            ...
            rapidjson::Value lineV;
            lineV.SetUint(getAsmLineAtInst(loc));
            rapidjson::Value argNoV;
            argNoV.SetUint(argVal->getArgNo());

            std::string rangeRes = disjointRangeSetToStr(argRng.second);
            rapidjson::Value rngV;
            rngV.SetString(rangeRes.c_str(), rangeRes.size(), allocator);
        ...
        for (auto bb : funRes.unreachableBBs) {
            rapidjson::Value locV;
            locV.SetUint(getAsmLineAtInst(&*bb->begin()));
...
// void WPInferenceInterface::dumpSynergy() {
//   rapidjson::Document dom;
//   dom.SetArray();
//   auto &allocator = dom.GetAllocator();
//
//   rapidjson::Value funRecord(rapidjson::kObjectType);
//
//   for (auto& ins : analysisResultByFun)
//
////  const Function *fun = iter->first;
////  const llvm::Instruction *firstInstOfFunc = &*fun->begin()->begin();
////  std::string funName = llvmFunToStr(iter->first);
...
PersistenceAnalysisData::deserialize(const llvm::Module *M, const std::string &resultJson) {
    ...
    for (auto &funRecord : dom.GetArray()) {
        ...
        assert(funRecord.HasMember("synRes"));
        for (auto &bugRes : funRecord["synRes"].GetArray()) {
            unsigned instLoc = bugRes["line"].GetUint();
            const Instruction *Inst = InstructionDbgInfo::lineToInst(instLoc);

            std::set<unsigned> group1, group2;
            assert(bugRes.HasMember("true") && bugRes.HasMember("false"));
            for (auto &bugID : bugRes["true"].GetArray()) {
                group1.insert(bugID.GetUint());
            }

            for (auto &bugID : bugRes["false"].GetArray()) {
                group2.insert(bugID.GetUint());
            }

            synergyResult[Inst] = std::make_pair(group1, group2);
        }
        ...
        WPInferenceInterface::FunctionResult funResult(argResult, valResult, synergyResult, {}, unreachableBBs);
