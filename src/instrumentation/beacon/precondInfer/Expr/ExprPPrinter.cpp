...
class PPrinter : public ExprPPrinter {
  ...
  private:
    ...
    bool isVerySimple(const ref<Expr> &e) { return isa<ConstantExpr>(e) || bindings.find(e) != bindings.end(); }

    bool isVerySimpleUpdate(const UpdateNode *un) { return !un || updateBindings.find(un) != updateBindings.end(); }
    ...
    void printUpdateList(const UpdateList &updates, PrintContext &PC) {
        ...
        for (const UpdateNode *un = head; un; un = un->next) {
            ...
            nextShouldBreak = !(isa<ConstantExpr>(un->index) && isa<ConstantExpr>(un->value));
        }
  ...
  public:
    ...
    void setForceNoLineBreaks(bool _forceNoLineBreaks) { forceNoLineBreaks = _forceNoLineBreaks; }
...
ExprPPrinter *klee::ExprPPrinter::create(llvm::raw_ostream &os) { return new PPrinter(os); }
...
struct ArrayPtrsByName {
    bool operator()(const Array *a1, const Array *a2) const { return a1->name < a2->name; }
};
}  // namespace
