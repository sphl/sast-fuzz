...
//------------------------------ HELPER FUNCTIONS ---------------------------//
bool ArrayExprHelper::isReadExprAtOffset(ref<Expr> e, const ReadExpr *base, ref<Expr> offset) {
...
ExprVisitor::Action ConstantArrayExprVisitor::visitConcat(const ConcatExpr &ce) {
    ...
    if (base) {
        ...
        if (base->updates.root->isConstantArray() && !isa<ConstantExpr>(base->index)) {
            ...
            if (compatible.isCompatible() && addedIndexes.find(base->index.get()->hash()) == addedIndexes.end()) {
                if (arrays.find(base->updates.root) == arrays.end()) {
                    arrays.insert(std::make_pair(base->updates.root, std::vector<ref<Expr> >()));
                } else {
...
ExprVisitor::Action ConstantArrayExprVisitor::visitRead(const ReadExpr &re) {
    ...
    if (re.updates.root->isConstantArray() && !isa<ConstantExpr>(re.index)) {
        ...
        if (compatible.isCompatible() && addedIndexes.find(re.index.get()->hash()) == addedIndexes.end()) {
            if (arrays.find(re.updates.root) == arrays.end()) {
                arrays.insert(std::make_pair(re.updates.root, std::vector<ref<Expr> >()));
            } else {
                // Another possible index to resolve, currently unsupported
                incompatible = true;
                return Action::skipChildren();
...
ExprVisitor::Action IndexCompatibilityExprVisitor::visitRead(const ReadExpr &re) {
    if (re.updates.head) {
        compatible = false;
        return Action::skipChildren();
    } else if (re.updates.root->isConstantArray() && !isa<ConstantExpr>(re.index)) {
        compatible = false;
        inner = true;
        return Action::skipChildren();
    }
...
// values twice
ExprVisitor::Action ArrayReadExprVisitor::inspectRead(unsigned hash, Expr::Width width, const ReadExpr &re) {
            ...
            if (re.updates.head) {
                ...
                for (const UpdateNode *un = re.updates.head; un; un = un->next) {
                    ...
                    if (!isa<ConstantExpr>(un->index)) {
                        ...
                        symbolic = true;
                    } else if (re.updates.root->isSymbolicArray() && isa<ConstantExpr>(un->value)) {
...
ExprVisitor::Action ArrayValueOptReplaceVisitor::visitConcat(const ConcatExpr &ce) {
