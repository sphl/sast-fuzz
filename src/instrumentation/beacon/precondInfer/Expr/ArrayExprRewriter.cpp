...
ref<Expr>
ExprRewriter::rewrite(const ref<Expr> &e, const array2idx_ty &arrays, const mapIndexOptimizedExpr_ty &idx_valIdx) {
    ref<Expr> notFound;
    ...
    for (auto &element : arrays) {
        ...
        for (std::vector<ref<Expr>>::const_iterator index_it = indexes.begin(); index_it != indexes.end(); ++index_it) {
                ...
                for (unsigned i = 0; i < arr->size / width; ++i) {
                    if ((!invert && ba.get(i)) || (invert && !ba.get(i))) {
                        ...
                        if (start >= 0) {
                            if (i - start == 1) {
                                eqExprs.push_back(
                                        createEqExpr((*index_it), ConstantExpr::create(start * width, idxWidth)));
                            } else {
                                // create range expr
    ...
    if (eqExprs.empty()) {
        ...
        if (isa<AndExpr>(eqExprs[0])) {
            return EqExpr::alloc(ConstantExpr::alloc(invert ? 0 : 1, (eqExprs[0])->getWidth()), eqExprs[0]);
        }
...
ref<Expr> ExprRewriter::concatenateOrExpr(const std::vector<ref<Expr>>::const_iterator begin,
    ...
    if (begin + 2 == end) {
        return OrExpr::alloc(ZExtExpr::alloc((*begin), Expr::Int32), ZExtExpr::alloc((*(begin + 1)), Expr::Int32));
    } else {
        return OrExpr::alloc(ZExtExpr::alloc((*begin), Expr::Int32), concatenateOrExpr(begin + 1, end));
    }
}

ref<Expr> ExprRewriter::createEqExpr(const ref<Expr> &index, const ref<Expr> &valIndex) {
    return EqExpr::alloc(valIndex, index);
}

ref<Expr> ExprRewriter::createRangeExpr(const ref<Expr> &index, const ref<Expr> &valStart, const ref<Expr> &valEnd) {
    return AndExpr::alloc(UleExpr::alloc(valStart, index), UleExpr::alloc(index, valEnd));
}
