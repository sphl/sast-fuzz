...
static u64 get_cpu_usage_us(void) {
    ...
    return (u.ru_utime.tv_sec * 1000000ULL) + u.ru_utime.tv_usec + (u.ru_stime.tv_sec * 1000000ULL) +
           u.ru_stime.tv_usec;
}
...
int main(int argc, char **argv) {
    ...
    ACTF("Measuring per-core preemption rate (this will take %0.02f sec)...", ((double)CTEST_CORE_TRG_MS) / 1000);
    ...
    if (idle_cpus) {
        if (maybe_cpus == idle_cpus) {
            SAYF(cLGN "PASS: " cRST "You can run more processes on %u core%s.", idle_cpus, idle_cpus > 1 ? "s" : "");

        } else {
            SAYF(cLGN "PASS: " cRST "You can run more processes on %u to %u core%s.", idle_cpus, maybe_cpus,
                 maybe_cpus > 1 ? "s" : "");
        }
    ...
    if (maybe_cpus) {
        SAYF(cYEL "CAUTION: " cRST "You may still have %u core%s available.", maybe_cpus, maybe_cpus > 1 ? "s" : "");
        SAYF(cGRA " <<<" cRST "\n\n");
        return 1;
    }
    ...
    ACTF("Measuring gross preemption rate (this will take %0.02f sec)...", ((double)CTEST_TARGET_MS) / 1000);
