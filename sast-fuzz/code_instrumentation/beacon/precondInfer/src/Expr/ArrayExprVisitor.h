...
class ConstantArrayExprVisitor : public ExprVisitor {
  ...
  public:
    explicit ConstantArrayExprVisitor(bindings_ty &_arrays) : arrays(_arrays), incompatible(false) {}
...
class IndexTransformationExprVisitor : public ExprVisitor {
  ...
  public:
    ...
    inline Expr::Width getWidth() { return width == Expr::InvalidWidth ? Expr::Int8 : width; }
...
class ArrayReadExprVisitor : public ExprVisitor {
  ...
  public:
    ArrayReadExprVisitor(std::vector<const ReadExpr *> &_reads,
                         std::map<const ReadExpr *, std::pair<unsigned, Expr::Width>> &_readInfo)
        : ExprVisitor(true), reads(_reads), readInfo(_readInfo), symbolic(false), incompatible(false) {}
    inline bool isIncompatible() { return incompatible; }
...
class ArrayValueOptReplaceVisitor : public ExprVisitor {
  ...
  public:
    explicit ArrayValueOptReplaceVisitor(std::map<unsigned, ref<Expr>> &_optimized, bool recursive = true)
