...
// operators
time::Point &time::Point::operator+=(const time::Span &span) {
    point += span.duration;
    return *this;
}
time::Point &time::Point::operator-=(const time::Span &span) {
...
time::Point time::operator+(const time::Point &point, const time::Span &span) {
    return time::Point(point.point + span.duration);
}
time::Point time::operator+(const time::Span &span, const time::Point &point) {
    return time::Point(point.point + span.duration);
}
time::Point time::operator-(const time::Point &point, const time::Span &span) {
    return time::Point(point.point - span.duration);
...
time::Span::Span(const std::string &s) {
    ...
    for (std::smatch m; std::regex_search(submatch, m, nre); submatch = m.suffix()) {
        ...
        Duration d;
        if (m[2] == "h")
            d = std::chrono::hours(value);
        else if (m[2] == "min")
            d = std::chrono::minutes(value);
        else if (m[2] == "s")
            d = std::chrono::seconds(value);
        else if (m[2] == "ms")
            d = std::chrono::milliseconds(value);
        else if (m[2] == "us")
            d = std::chrono::microseconds(value);
        else if (m[2] == "ns")
            d = std::chrono::nanoseconds(value);
        else
            goto error;
...
// operators
time::Span &time::Span::operator=(const time::Duration &d) {
    duration = d;
    return *this;
};
time::Span &time::Span::operator+=(const time::Span &other) {
...
time::Span &time::Span::operator*=(const time::Duration::rep &rep) {
    duration *= rep;
    return *this;
}

time::Span time::operator+(const time::Span &lhs, const time::Span &rhs) {
    return time::Span(lhs.duration + rhs.duration);
}
time::Span time::operator-(const time::Span &lhs, const time::Span &rhs) {
    return time::Span(lhs.duration - rhs.duration);
}
time::Span time::operator*(const time::Span &span, const time::Duration::rep &rep) {
    return time::Span(span.duration * rep);
}
time::Span time::operator*(const time::Duration::rep &rep, const time::Span &span) {
    return time::Span(span.duration * rep);
}
time::Span time::operator/(const time::Span &span, const time::Duration::rep &rep) {
    return time::Span(span.duration / rep);
...
llvm::raw_ostream &time::operator<<(llvm::raw_ostream &stream, time::Span span) {
    return stream << span.toSeconds() << 's';
}
...
std::string time::getClockInfo() {
    ...
    buffer << "Using monotonic steady clock with " << std::chrono::steady_clock::period::num << '/'
           << std::chrono::steady_clock::period::den << "s resolution\n";
    return buffer.str();
}
...
time::Point time::getWallTime() { return time::Point(std::chrono::steady_clock::now()); }
