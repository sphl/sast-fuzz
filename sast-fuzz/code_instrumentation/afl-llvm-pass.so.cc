...
class AFLCoverage : public ModulePass {
  public:
    ...
    // StringRef getPassName() const override { return "American Fuzzy Lop Instrumentation"; }
};
...
bool AFLCoverage::runOnModule(Module &M) {
    ...
    GlobalVariable *AFLMapPtr = (GlobalVariable *)M.getOrInsertGlobal(
            "__afl_area_ptr", PointerType::get(Int8Ty, 0), []() -> GlobalVariable * {
    ...
    GlobalVariable *AFLPrevLoc =
            new GlobalVariable(M, Int32Ty, false, GlobalValue::ExternalLinkage, nullptr, "__afl_prev_loc", nullptr,
    ...
    if (!be_quiet) {
        if (!inst_blocks)
            ...
            OKF("Instrumented %u locations (%s mode, ratio %u%%).", inst_blocks,
                getenv("AFL_HARDEN")
                        ? "hardened"
                        : ((getenv("AFL_USE_ASAN") || getenv("AFL_USE_MSAN")) ? "ASAN/MSAN" : "non-hardened"),
                inst_ratio);
    }
...
static void registerAFLPass(const PassManagerBuilder &, legacy::PassManagerBase &PM) { PM.add(new AFLCoverage()); }
