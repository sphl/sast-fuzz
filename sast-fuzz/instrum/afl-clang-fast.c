...
static void find_obj(u8 *argv0) {
    ...
    if (afl_path) {
        tmp = alloc_printf("%s/afl-llvm-rt.a", afl_path);
    ...
    if (slash) {
        ...
        *slash = '/';
        ...
        tmp = alloc_printf("%s/afl-llvm-rt.a", dir);
    ...
    if (!access(AFL_PATH "/afl-llvm-rt.a", R_OK)) {
        obj_path = AFL_PATH;
        return;
    }

    FATAL("Unable to find 'afl-llvm-rt.a' or 'afl-llvm-pass.so'. Please set AFL_PATH");
}
...
static void edit_params(u32 argc, char **argv) {
    ...
    if (maybe_linking) {
        ...
        switch (bit_mode) {
        case 0:
            cc_params[cc_par_cnt++] = alloc_printf("%s/afl-llvm-rt.a", obj_path);
            break;
