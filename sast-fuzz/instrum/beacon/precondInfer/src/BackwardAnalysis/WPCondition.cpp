...
namespace {
cl::opt<unsigned> JoinBound("join-bound", cl::init(20));
cl::opt<unsigned> WidenBound("widen-bound", cl::init(2));

cl::opt<unsigned> PtsBound("pts-bound", cl::init(5));

cl::opt<std::string> DebugFun("debug-func", cl::init(""), cl::desc("Specify a function name for debugging"));
...
class ExprSafeReplaceVisitor : public ExprVisitor {
  ...
  public:
    bool errorOccured() const { return hasError; }
  ...
  public:
    Action visitSDiv(const SDivExpr &e) { return visitDivRemExpr(e); }

    Action visitUDiv(const UDivExpr &e) { return visitDivRemExpr(e); }

    Action visitURem(const URemExpr &e) { return visitDivRemExpr(e); }

    Action visitSRem(const SRemExpr &e) { return visitDivRemExpr(e); }
};
...
class ExprReplaceVisitor2 : public ExprSafeReplaceVisitor {
  ...
  public:
    ExprReplaceVisitor2(const std::map<SymExpr, SymExpr> &_replacements)
        : ExprSafeReplaceVisitor(true), replacements(_replacements) {}

    Action visitExprPost(const Expr &e) {
        std::map<SymExpr, SymExpr>::const_iterator it = replacements.find(SymExpr(const_cast<Expr *>(&e)));
...
bool isExprContainSubExpr(SymExpr src, SymExpr dst) {
    std::function<bool(SymExpr)> pred = [dst](SymExpr exp) { return exp == dst; };
...
std::shared_ptr<ICFGNode> PreconditionFixpo::getNodePost(const llvm::Instruction *inst) { return icfg.getNode(inst); }

PreconditionFixpo::PreconditionFixpo(const Instruction *target, const ICFG &icfg, bool interAnalysis)
    ...
    //  AA.registerFunctionAnalysis<llvm::DominatorTreeWrapperPass>();
    //  //  AA.registerFunctionAnalysis<llvm::CFLAndersAA>();
    //  FAM.registerPass(llvm::DominatorTreeWrapperPass());
...
unsigned PreconditionFixpo::getWidthForType(llvm::Type *ty) const { return DL->getTypeSizeInBits(ty); }
...
SymExpr PreconditionFixpo::getTrueCond() const { return klee::ConstantExpr::create(1, 1); }

SymExpr PreconditionFixpo::getFalseCond() const { return klee::ConstantExpr::create(0, 1); }

std::pair<std::set<std::shared_ptr<ICFGNode>>, std::set<const llvm::Function *>>
PreconditionFixpo::getReachableNodesForTarget() const {
...
void PreconditionFixpo::addValSym(const Value *val, const SymExpr &sym) { valSym.insert({val, sym}); }
...
SymExpr
PreconditionFixpo::generateSymbol(Expr::Width width, const std::string &category, const std::string &suffix) const {
    ...
    assert(width <= 64);
    if (width == Expr::Bool) {
        return ZExtExpr::create(ReadExpr::create(ul, klee::ConstantExpr::alloc(0, Expr::Int32)), Expr::Bool);
    }
    ...
    while (remainingWidth >= 8) {
        SymExpr cur = ReadExpr::create(ul, klee::ConstantExpr::alloc(byteIdx, Expr::Int32));
        ++byteIdx;
    ...
    if (remainingWidth != 0) {
        SymExpr cur =
                ZExtExpr::create(ReadExpr::create(ul, klee::ConstantExpr::alloc(byteIdx, Expr::Int32)), remainingWidth);
        components.push_back(cur);
    }
...
/// For instruction x = expr, where x is a tracked variable, expr is not tracked
/// by the analysis The precondition transformer will make x unconstrained
...
SymExpr PreconditionFixpo::transferBinaryArithInst(const Instruction &I, SymExpr cond) {
    ...
    SymExpr rhsExp;
    assert(OpC == Instruction::Add || OpC == Instruction::Sub || OpC == Instruction::Mul || OpC == Instruction::SDiv ||
           OpC == Instruction::UDiv || OpC == Instruction::SRem || OpC == Instruction::URem);

    if ((!isValueTracked(I.getOperand(0))) || (!isValueTracked(I.getOperand(1)))) {
        /// possible: %192 = sub i64 %191, ptrtoint ([394 x i8*]* @htmlStartClose to
        /// i64), !dbg !12669
...
SymExpr PreconditionFixpo::transferBinaryBitwiseInst(const Instruction &I, SymExpr cond) {
    ...
    auto OpC = I.getOpcode();
    SymExpr rhsExp;
    assert(OpC == Instruction::Shl || OpC == Instruction::LShr || OpC == Instruction::AShr || OpC == Instruction::And ||
...
SymExpr PreconditionFixpo::transferGEPInst(const Instruction &I, SymExpr cond) {
    ...
    for (auto it = indices.begin(), ie = indices.end(); it != ie; ++it) {
        ...
        rhsExp = AddExpr::create(rhsExp, MulExpr::create(SExtExpr::create(index, PointerSizeInBits),
...
SymExpr PreconditionFixpo::transferCallInst(const Instruction &I, SymExpr cond) {
    ...
    SVF::CallSite cs(const_cast<Instruction *>(&I));
    bool overApprox = false;
    if (!isa<DbgInfoIntrinsic>(&I) && !SVF::SVFUtil::isObject(&I)) {  // e.g., lib fun
        overApprox = true;
    }
...
SymExpr PreconditionFixpo::transferSelectInst(const Instruction &I, SymExpr cond) {
    ...
    assert(isValueTracked(inst->getCondition()) && isValueTracked(inst->getTrueValue()) &&
           isValueTracked(inst->getFalseValue()));
...
SymExpr PreconditionFixpo::transferCastInst(const Instruction &I, SymExpr cond) {
    ...
    if (OpC == Instruction::Trunc) {
        rhsExp = ExtractExpr::create(srcExp, 0, getWidthForType(I.getType()));
    } else if (OpC == Instruction::ZExt) {
        rhsExp = ZExtExpr::create(srcExp, getWidthForType(I.getType()));
    } else if (OpC == Instruction::SExt) {
        rhsExp = SExtExpr::create(srcExp, getWidthForType(I.getType()));
...
std::vector<SymExpr> PreconditionFixpo::transferInstData(const Instruction &I, SymExpr cond) {
    ...
    for (unsigned i = 0; i < I.getNumOperands(); ++i) {
        ...
        if (opnd->getType()->isIntegerTy() && !isValueTracked(opnd)) {
            /// There are some gross cases like: %11 = or i1 %10, icmp ne (i64 and
            /// (i64 ptrtoint ([0 x i32]* @j
            ///                                                     const_rgb_gray_convert_avx2
            ///                                                     to i64), i64 31),
            ///                                                     i64 0), !dbg !7595
            return {replaceLHSWithUnknownTemp(I, cond)};
        }
    ...
    if (OpC == Instruction::Add || OpC == Instruction::Sub || OpC == Instruction::Mul || OpC == Instruction::SDiv ||
        OpC == Instruction::UDiv || OpC == Instruction::SRem || OpC == Instruction::URem) {
        return {transferBinaryArithInst(I, cond)};
    } else if (OpC == Instruction::Shl || OpC == Instruction::LShr || OpC == Instruction::AShr ||
               OpC == Instruction::And || OpC == Instruction::Or || OpC == Instruction::Xor) {
        ...
        // other untracked inst producing a tracked symbol, such as: i =
...
/// ret --> exit (of fun)
SymExpr
PreconditionFixpo::transferRetToExit(const llvm::Instruction &callI, const llvm::Instruction &retI, SymExpr cond) {
    assert(icfg.isTrackedCallInst(&callI));
    const Function *fun = retI.getFunction();
    SVF::CallSite cs(const_cast<Instruction *>(&callI));
...
void PreconditionFixpo::transferLocalControl(const std::shared_ptr<ICFGNode> &src,
                                             const std::shared_ptr<ICFGNode> &dst,
                                             const SymExpr &cond) {
    ...
    if (const auto *bI = dyn_cast<BranchInst>(predBBTI)) {
        if (bI->isConditional() && isValueTracked(bI->getCondition())) {  /// there are cases like br i1 icmp ne (i32
                                                                          /// (i32*, void ()*)* @pthread_once, i32
                                                                          /// (i32*, void ()*)* null), label %32, label
                                                                          /// %257, !dbg !202894
...
bool PreconditionFixpo::updateStateOut(const std::shared_ptr<ICFGNode> &loc,
                                       const SymExpr &cond,
...
bool PreconditionFixpo::updateState(const std::shared_ptr<ICFGNode> &loc,
                                    const SymExpr &cond,
                                    bool isPostCond,
    ...
    if (!joinedLocs.count(loc) && curSummary.size() < JoinBound) {
        ...
        std::shared_ptr<AbstractState> accumulateState =
                std::make_shared<AbstractState>(*symbolicAbstract(curSummary[0]));
        ...
        for (unsigned BugIter = 0; BugIter < CurID; ++BugIter) {
            std::shared_ptr<AbstractState> previousState =
                    std::make_shared<AbstractState>(*symbolicAbstract(StateSummary[BugIter][loc][0]));

            if (isPostCond) {
                if (!joinedOutSummary[BugIter].count(loc)) {
                    for (unsigned i = 1; i < StateSummary[BugIter][loc].size(); ++i) {
                        std::shared_ptr<AbstractState> abs = symbolicAbstract(StateSummary[BugIter][loc][i]);
                        previousState->join(*abs);
                    }
                }
            } else {
                if (!joinedInSummary[BugIter].count(loc)) {
                    for (unsigned i = 1; i < StateSummary[BugIter][loc].size(); ++i) {
                        std::shared_ptr<AbstractState> abs = symbolicAbstract(StateSummary[BugIter][loc][i]);
                        previousState->join(*abs);
                    }
            ...
            auto syn = absNew->getIntersect(*previousState);

            for (auto &Val : syn.OverlapVal) {
                if (ResCluster.find(Val) == ResCluster.end()) {
                    std::set<unsigned> tmp1 = {CurID, BugIter};
                    std::set<unsigned> tmp2 = {};
                    ResCluster[Val] = std::make_pair(tmp1, tmp2);
                } else {
                    if (ResCluster[Val].first.find(BugIter) != ResCluster[Val].first.end()) {
                        ResCluster[Val].first.insert(CurID);
                    } else {
                        ResCluster[Val].second.insert(CurID);
                    }
                }
            }

            for (auto &Val : syn.ContradictVal) {
                if (ResCluster.find(Val) == ResCluster.end()) {
                    std::set<unsigned> tmp1 = {CurID};
                    std::set<unsigned> tmp2 = {BugIter};
                    ResCluster[Val] = std::make_pair(tmp1, tmp2);
                } else {
                    if (ResCluster[Val].first.find(BugIter) != ResCluster[Val].first.end()) {
                        ResCluster[Val].second.insert(CurID);
        ...
        if (isPostCond) {
            ...
            if (Contradict.empty()) {
                std::map<unsigned, llvm::SmallVector<SymExpr, 8>> tmpBound;
                tmpBound[0] = {combined};
                Contradict[loc] = tmpBound;
            } else {
                Contradict[loc][group] = {combined};
            }
            /// There could be postconditions heading for different dst blocks merged
            /// in "combined" Thus, the merged condition itself does not have a
...
void PreconditionFixpo::debugFunc(const std::shared_ptr<ICFGNode> &loc,
                                  SymExpr cond,
    ...
    if (loc->getKind() == ICFGNode::NORMAL_NODE) {
        if (loc->getInst()->getPrevNode() &&
            loc->getInst()->getNextNode()) {  // only log first/last instruction of a bb
            return;
        }
    }
...
void PreconditionFixpo::run() {
        ...
        if (locKind == ICFGNode::NORMAL_NODE) {
            ...
            if (inst->getOpcode() == Instruction::PHI && isValueTracked(inst)) {
                ...
                /*for (unsigned i = 0; i < phiInst->getNumIncomingValues(); ++i) {
                  ...
                  for (auto nextIter = locNode->next_begin(), eIter =
                locNode->next_end(); nextIter != eIter; ++nextIter) { auto nextICFGNode
                = *nextIter; assert(nextICFGNode->getFunc() == locNode->getFunc());
                ...
                auto getPhiPrecondForBB = [phiInst, this, &phiSym, inst](const SymExpr &post,
                ...
                if (isFirstInstInBB) {  // first phi
                    if (dstBB) {
                        ...
                        for (auto nextIter = locNode->next_begin(), eIter = locNode->next_end(); nextIter != eIter;
                             ++nextIter) {
                ...
                } else {  // There are a sequence of phis, and this phi inst is not the
                          // first one
            ...
            for (auto nextIter = locNode->next_begin(), eIter = locNode->next_end(); nextIter != eIter; ++nextIter) {
                ...
                // there can be function with no return instruction, e.g., wrapper for
                // exit functions. in such cases, the backward propagation stops
            ...
            {
                SVF::CallSite cs(const_cast<Instruction *>(locNode->getInst()));
                if (cs.getCalledFunction() == locNode->getFunc()) {
                    /// The edge between CALL & RET is crucial to handle recursive
                    /// function. See IFDS's paper.
    ...
    StateSummary[CurID] = stateOutMap;
    joinedOutSummary[CurID] = joinedLocOut;
    joinedInSummary[CurID] = joinedLocIn;
    CurID++;
}

std::unordered_map<const llvm::Function *, PreconditionFixpo::FixpointDisjunctiveResult>
PreconditionFixpo::getAnalysisResult() {
    ...
    for (auto fun : reachableFuncs) {
        ...
        for (auto bbIter = fun->begin(), eIter = fun->end(); bbIter != eIter; ++bbIter) {
            ...
            for (auto instIter = bb->begin(); instIter != bb->end(); ++instIter) {
                ...
                if (ResCluster.find(inst) != ResCluster.end()) {
                    funRes.setSynResult(inst, std::move(ResCluster[inst]));
                }
            }
        }
    ...
    llvm::errs() << "find syn: " << ResCluster.size() << "\n";
    return res;
}
...
std::pair<std::vector<const Value *>, std::vector<SymExpr>>
PreconditionFixpo::getConstrainedValSym(SymExpr cond) const {
    std::function<bool(SymExpr)> pred = [this](const SymExpr &exp) { return symToIntVals.count(exp); };
...
std::vector<SymExpr> PreconditionFixpo::getConstrainedMemSym(const SymExpr &cond) const {
    std::function<bool(SymExpr)> pred = [this](const SymExpr &exp) { return memSymToLdPtr.count(exp); };
