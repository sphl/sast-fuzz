...
class generic_gep_type_iterator
  ...
  typedef std::iterator<std::forward_iterator_tag, llvm::Type *, ptrdiff_t>
    ...
    ItTy OpIt;
    llvm::PointerUnion<llvm::StructType *, llvm::Type *> CurTy;
    generic_gep_type_iterator() {}

    llvm::Value *asValue(llvm::Value *V) const { return V; }
    llvm::Value *asValue(unsigned U) const {
      return llvm::ConstantInt::get(CurTy.dyn_cast<llvm::Type *>()->getContext(), llvm::APInt(32, U));
    }
  ...
  public:
    ...
    llvm::Type *operator*() const { return CurTy.dyn_cast<llvm::Type *>(); }

    llvm::Value *getOperand() const { return asValue(*OpIt); }

    llvm::Type *getIndexedType() const {
      if (auto *T = CurTy.dyn_cast<llvm::Type *>()) {
        return T;
      }
      return CurTy.get<llvm::StructType *>()->getTypeAtIndex(getOperand());
    }

    generic_gep_type_iterator& operator++() { // Preincrement
      llvm::Type *Ty = getIndexedType();
      ...
      if (auto *ATy = dyn_cast<llvm::ArrayType>(Ty)) {
        ...
        CurTy = dyn_cast<llvm::StructType>(Ty);
      }
      ++OpIt;
